...
pair_id_val <- unique(msmc_pseudo$pair_id)[1]

pseudo_pair <- msmc_pseudo %>%
  filter(pair_id == pair_id_val)
...
detect_msmc_spike <- function(psmc, mu, gen, segment = 25) {
  ...
  return(df$time_scaled[segment])
}



spike_value <- detect_msmc_spike(
  psmc     = pseudo_pair,
  mu       = pseudo_mu,
  gen      = gen,
)
...
spike_value
# numeric time in years (already scaled)
...
ggsave(
  filename = file.path(plots_dir, "msmc_pseudo_lines_test.png"),
  plot     = p_spike,
  width    = 10,
  height   = 7,
  dpi      = 300
...
detect_msmc_spike <- function(msmc, mu, gen,
                              callSpikeThreshold = 0.10,
                              min_time = 2e4,
                              debug = FALSE) {

  msmc <- msmc %>%
    arrange(left_time_boundary) %>%
    mutate(time_scaled = (left_time_boundary / mu) * gen)

  # Only bins after min_time
  valid_idx <- which(msmc$time_scaled >= min_time)
  if (length(valid_idx) < 3) return(NA_real_)

  if (debug) {
    message("First bins after min_time:")
    print(msmc[valid_idx[1]:min(valid_idx[1]+20, max(valid_idx)), 
               c("left_time_boundary", "right_time_boundary", "lambda_00", "time_scaled")])
  }

  # -----------------------------
  # Step: Find the first local maximum after min_time
  # -----------------------------
  for (k in valid_idx[-c(1, length(valid_idx))]) {
    # k moves LEFT → RIGHT through time bins (from more recent to older times)
    # valid_idx ensures we only consider bins beyond min_time
    # -c(1, length(valid_idx)) prevents out-of-bounds checks for neighbors

    # Local maximum condition:
    #   - The current bin is higher than the previous (closer to present)
    #   - AND higher than the next bin (further back in time)
    if (msmc$lambda_00[k] > msmc$lambda_00[k-1] &&
        msmc$lambda_00[k] > msmc$lambda_00[k+1]) {

      # -----------------------------
      # Step: Mark the detected peak
      # -----------------------------
      peak_lambda <- msmc$lambda_00[k]  # lambda at the local maximum
      if (debug) message(
        "Detected peak at ~", 
        round(msmc$time_scaled[k]), 
        " yrs (λ = ", round(peak_lambda), ")"
      )
      # At this point, we've found where the Ne starts to rise relative to neighbors

      # -----------------------------
      # Step: Backtrack to find ≥10% drop
      # -----------------------------
      # We go backwards (right → left in the vector, which is older → more recent)
      # Only consider bins that come BEFORE the peak (valid_idx < k)
      back_idx <- rev(valid_idx[valid_idx < k])

      # Find the first bin where lambda_00 is at least 10% smaller than the peak
      # i.e., lambda <= peak / (1 + callSpikeThreshold)
      j <- back_idx[which(msmc$lambda_00[back_idx] <= peak_lambda / (1 + callSpikeThreshold))[1]]

      if (!is.na(j)) {
        # Convert the bin’s right edge into years
        # RIGHT_EDGE is used because MSMC step functions are constant within a bin
        spike <- (msmc$right_time_boundary[j] / mu) * gen
        message("Inferred spike at (years): ", round(spike))
        return(spike)  # stop at the first qualifying backtrack point
      }

      # -----------------------------
      # Fallback: no qualifying drop found
      # -----------------------------
      # If none of the earlier bins are low enough, just use the peak itself
      spike <- (msmc$left_time_boundary[k] / mu) * gen
      message("Inferred spike at peak (no backtrack): ", round(spike))
      return(spike)
    }
  }
                              }
...
# Add vertical line in ggplot
p_spike <- p + geom_vline(
    xintercept = spike_value,
    linetype = "longdash",
    linewidth = 1,
    colour = "darkred"
...
ggsave(
  filename = file.path(plots_dir, "msmc_spike_test.png"),
  plot     = p_spike,
  width    = 10,
  height   = 7,
  dpi      = 300
...
for (pair_id in pairs_to_run) {
  ...
  # NEW: extract regions for naming
  reg1 <- pseudo_pair$region1[1]
  reg2 <- pseudo_pair$region2[1]
  ...
  # UPDATED: save file using region names. can do with sample names instead
  save_file <- file.path(
  ...
  plot_msmc_facet(
    ...
    # 1️⃣ Build base plot (DO NOT save yet)
  p <- plot_msmc_facet(
    msmc_data = autosomes_sub,
    autosomal_data = autosomes_sub,
    pseudodiploid_data = pseudo_pair,
    ...
    color_var = "continent",
    line_label = "sample + region",
    save_file = NULL,          # important
    xlims = c(3e4, 5e6),
    ylims = c(1e3, 1e12)
  )

  # 2️⃣ Detect spike for THIS pseudodiploid
  spike_value <- detect_msmc_spike(
    psmc      = pseudo_pair,
    mu        = pseudo_mu,
    gen       = gen,
  ...
  # 3️⃣ Add spike line (if one exists)
  if (!is.na(spike_value)) {
    p <- p +
      geom_vline(
        xintercept = spike_value,
        linetype   = "longdash",
