...
# ----------- Read sample order from .fam file and make a tibble from it -----------
#have to be very careful with the .fam file data as the columns are not labeled and are not
#obvious to interpret
# The .fam file's second column contains sample IDs, which matches the row order in Q files.
# eg 0	AK_S12-1159	0	0	0	-9
#0	ECAN_S14-0358	0	0	0	-9

# Read .fam sample order
### ADMIXTOOLS / ADMIXTURE plotting
# ----------- Read .fam file sample order -----------
...
# Combine sample + continent into a tibble and print first 20 rows
...
# range of K values you want to plot
k_values <- 3:total_k  # or whatever your total_k is
# Generate full paths to Q files based on K
q_files <- file.path(q_file_base, paste0("CLUMPAK.", k_values, ".Q"))

# Optional: check that all files exist
q_files[!file.exists(q_files)]  # should return character(0) if all exist
...
# Assume you have q_files and k_values defined (e.g., q_files = list.files(...), k_values = c(...))
admix_plot_list <- map2(q_files, k_values, \(f, k) if (k <= total_k) plot_admix(f, k)) %>%
  compact()

# ----------- Combine plots with only last showing labels and single legend at bottom -----------
...
combined_plot <- map2(admix_plot_list, seq_len(n), \(p, i) {
  if (i < n) {
    # All but last plot: remove labels and legend
    p + theme(
    ...
    # Last plot: add region labels and leave space at bottom
    p +
      geom_text(data = label_df, aes(x = sample, y = -0.01, label = region,
                                     color = continent),
      ...
      theme(
        ...
        plot.margin = margin(20, 20, 80, 20)  # increased bottom margin for country names
      )
  }
}) %>%
...
combined_plot <- map2(admix_plot_list, seq_len(n), \(p, i) {
  label_layer <- if (i == n) {
    geom_text(data = label_df, aes(x = sample, y = -0.02, label = region, color=continent),
              angle = 45, hjust = 1, size = 4, inherit.aes = FALSE)
  } else NULL #only add text labels to the very last plot
  ...
  theme(                 # override/add theme elements
    ...
    plot.margin = margin(10, 20, ifelse(i == n, 80, 5), 20)  
