...
##############################################################################
#include ellie armstrong assembly
##############################################################################
...
metadata4 <- metadata %>%
  anti_join(
    tibble(sample = additional_samples),
    by = "sample"
  ) %>%
  bind_rows(
    tibble(
...
evec_path <- "/gpfs/data/bergstrom/paula/fox_repo/variant_calling/Eigenstrat/armstrong_vvulpes_eigenstrat/armstrong_vvulpes_tidy.evec"
eval_path <- "/gpfs/data/bergstrom/paula/fox_repo/variant_calling/Eigenstrat/armstrong_vvulpes_eigenstrat/armstrong_vvulpes.pca.eval"
...
EA_plot <- plot_pca(
  ...
  metadata = metadata4,
  point_label = "sample ",
  output_file = "/gpfs/bio/xrq24scu/fox_repo/plotting/EA.png",
  plot_title = "PCA including EA assembly",
  continent_colors = continent_colors,
  width = 30,
  height = 20
)

EA_plot_poster <- plot_pca(
  evec_path = evec_path,
  eval_path = eval_path,
  metadata = metadata4,
  point_label = "sample + region",
  output_file = "/gpfs/bio/xrq24scu/fox_repo/plotting/EA.poster.png",
  plot_title = "PCA including EA assembly",
  continent_colors = continent_colors,
  poster = TRUE
)
...
detect_msmc_spike_lambda <- function(
  ...
  if (debug) {
    message("MSMC table (ancient → recent):")
    print(msmc_proc %>% select(time_scaled, lambda))
  }

  n <- nrow(msmc_proc)
  
  # ---- storage for candidate spikes ----
  spikes <- tibble(
    type = character(),
    time = numeric(),
    ratio = numeric()
  ...
  # ---- iterate ancient → recent ----
  for (i in 2:n) {
    lambda_prev <- msmc_proc$lambda[i - 1]
    lambda_curr <- msmc_proc$lambda[i]
    
    # --- single-bin ratio ---
    if (!is.na(lambda_prev) && !is.na(lambda_curr) &&
        !is.null(single_bin_threshold)) {
      lambda_ratio <- lambda_prev / lambda_curr
      if (lambda_ratio >= single_bin_threshold) {
        spikes <- spikes %>% add_row(
          type = "single_bin",
          time = msmc_proc$time_scaled[i],
          ratio = lambda_ratio
        )
        if (debug) message("Single-bin spike candidate at ",
                           signif(msmc_proc$time_scaled[i], 4),
                           " (ratio: ", signif(lambda_ratio, 4), ")")
      }
    }
    
    # --- multi-bin ratio ---
    if (!is.null(multi_bin_bins) && !is.null(multi_bin_threshold) &&
        i + multi_bin_bins <= n) {
      lambda_start <- msmc_proc$lambda[i]
      lambda_end   <- msmc_proc$lambda[i + multi_bin_bins]
      if (!is.na(lambda_start) && !is.na(lambda_end) &&
          lambda_start > 0) {
        lambda_ratio_multi <- lambda_start / lambda_end
        if (lambda_ratio_multi >= multi_bin_threshold) {
          spikes <- spikes %>% add_row(
            type = "multi_bin",
            time = msmc_proc$time_scaled[i],
            ratio = lambda_ratio_multi
          )
          if (debug) message("Multi-bin spike candidate from ",
                             signif(msmc_proc$time_scaled[i + multi_bin_bins],4),
                             " → ", signif(msmc_proc$time_scaled[i],4),
                             " (ratio: ", signif(lambda_ratio_multi,4), ")")
        }
      }
    }
  }
  
  # ---- choose spike to return ----
  if (nrow(spikes) == 0) {
    if (debug) message("No spikes detected.")
    return(NULL)
  }
  
  # prioritize multi-bin spikes first
  multi_spikes <- spikes %>% filter(type == "multi_bin")
  if (nrow(multi_spikes) > 0) {
    # oldest multi-bin spike (ancient → recent)
    oldest_multi <- multi_spikes %>% slice(1)
    if (debug) message("Returning oldest multi-bin spike at ", oldest_multi$time)
    return(oldest_multi$time)
  }
  
  # if no multi-bin, return oldest single-bin spike
  oldest_single <- spikes %>% filter(type == "single_bin") %>% slice(1)
  if (debug) message("Returning oldest single-bin spike at ", oldest_single$time)
  return(oldest_single$time)
}
```
...
```{r}
debug_pairs <- c(
  "AK_S12-1159_LIS1300",#ka × England example
  "AK_S12-1159_LIS557"       # Alaska × Finland example
)
for (pattern in debug_pairs) {
  pseudo_pair <- msmc_pseudo %>% filter(grepl(pattern, pair_id))

  if (nrow(pseudo_pair) == 0) {
    message("No data for pattern: ", pattern)
    next
  }

  message("\n=== DEBUG: ", pseudo_pair$pair_id[1], " ===\n")

  msmc_proc <- pseudo_pair %>%
    mutate(
      lambda = lambda_00,
      time_scaled = right_time_boundary / pseudo_mu * gen
    ) %>%
    arrange(desc(time_scaled))  # ancient → recent

  # Inspect around ~100kya
  window_mask <- msmc_proc$time_scaled >= 5e4 & msmc_proc$time_scaled <= 1.5e5
  msmc_window <- msmc_proc[window_mask, ]
  print(msmc_window %>% select(time_scaled, lambda))
  ...
  spike_value <- detect_msmc_spike_lambda(
    ...
    gen = gen,
    multi_bin_bins = 4,
    multi_bin_threshold = 25,
    single_bin_threshold = 11000,
    debug = TRUE
  )

  if (!is.null(spike_value)) {
    message("Spike detected at: ", signif(spike_value, 4), " years")
  } else {
    message("No spike detected in this pair around 100kya.")
  }
}
...
```{r}
#just a helper chunk to make everything above easier to run 
```
