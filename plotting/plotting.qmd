...
# ----------- Filter to samples used in heterozygosity plot -----------
valid_samples <- filtered_cov_het$sample
...
# ----------- Axis labels: sample (continent) -----------
axis_labels <- sample_order_df %>%
  mutate(label = paste0(sample, " (", continent, ")")) %>%
  select(sample, label)
...
fst_long <- fst_long %>%
  filter(sample1 != sample2) %>%
  mutate(
    x_lab = label_vec[sample1],
    y_lab = label_vec[sample2]
  )
...
fst_plot <- ggplot(
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "Pairwise FST (filtered to heterozygosity samples)",
    x = NULL,
    y = NULL
  ) +
  theme(
...
sample_order_df <- tibble(sample = rownames(fst_mat)) %>%
  ...
  left_join(
    order_df %>% mutate(admix_order = row_number()),
    by = "sample"
  )

# Final ordering:
#   1) continent_order
#   2) ADMIXTURE order within continent
...
fst_long <- fst_long %>% filter(sample1 != sample2)
...
fst_plot <- ggplot(
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "Pairwise FST between samples",
    x = NULL,
    y = NULL
  ) +
  theme(
...
ggsave(
  "/gpfs/bio/xrq24scu/fox_repo/plotting/pairwise_fst_heatmap.png",
  fst_plot,
  width = 12,
  height = 10,
  dpi = 300
...
plot_msmc_facet <- function(
  ...
  if (!is.null(line_label)) {
    # label_df <- df %>%
    #   #filter(!is_pseudodiploid) %>%   # <-- TEMPORARILY REMOVE THE PSEUDODIPLOID LABEL FOR THE CONFERENCE
    ...
    label_df <- df %>%
          ...
          slice_max(time_scaled, n = 1, with_ties = FALSE) %>%
          ungroup() %>%
          mutate( # Push labels slightly to the right so segments are visible 
          label_x = time_scaled * .85, label_y = Ne_scaled )
        ...
        p <- p + ggrepel::geom_text_repel(
          ...
          box.padding = 1.5,
          point.padding = 0.2,
          segment.color = "grey60", # leader lines
          segment.size = 1,
          segment.alpha = 0.4,
          show.legend = FALSE
        )
  ...
  if (!is.null(save_file)) {
    ggsave(save_file, p, width = 10, height = 8, dpi = 300)
  }
