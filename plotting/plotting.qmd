...
MSMC_PLOT <- ggplot(
  ...
  scale_color_manual(values = continent_colors) +   # <--- reuse the same colors
  scale_x_log10(
...
plot_MSMC_facets <- function(msmc_data, continent_colors, mu, gen,
  ...
  # Optionally filter for highlighted samples
  if (!is.null(highlight_samples)) {
    msmc_data <- msmc_data %>%
      filter(sample %in% highlight_samples)
  }
  
  # Generate labels: leftmost point per sample
  labels_df <- msmc_data %>%
    group_by(sample, continent, region) %>%
    slice_min((left_time_boundary / mu) * gen, n = 1) %>%
    ungroup()
...
# Full plot with all samples
full_plot <- plot_MSMC_facets(msmc_allsamples_annotated, continent_colors, mu, gen)

# Highlight plot with only specific samples
highlight_samples_list <- c("LIS935", "S212760", "S120237", "ORC_S13-2559")
highlight_plot <- plot_MSMC_facets(msmc_allsamples_annotated, continent_colors, mu, gen,
                                   highlight_samples = highlight_samples_list)

# Save plots
ggsave(file.path(MSMC_allsamples_dir, "msmc_allsamples_faceted.png"),
       full_plot, width = 20, height = 20, dpi = 300)

ggsave(file.path(MSMC_allsamples_dir, "msmc_highlight_samples_faceted.png"),
