...
plot_msmc_facet <- function(
  ...
  mu,
  gen,
  pseudodip_mu = NULL,          # unchanged
  panel_grouping_var,
  color_var,
  group_line_var,
  line_label = NULL,
  overlay_autosomes = FALSE,
  autosomal_data = NULL,
  manual_colors = NULL,
  xlims = NULL,
  ylims = NULL,
  ### REMOVED: highlight_var, highlight_values
  ncol_panels = 3,
  save_file = NULL
) {

  # Only check line_label if not NULL
  ...
  ### CHANGED: restored correct MSMC variable name lambda_00
  df <- msmc_data %>%
    mutate(
  ...
  ### CHANGED: correct pseudodiploid scaling again uses lambda_00
  if (!is.null(pseudodip_mu)) {
    df <- df %>%
      mutate(
        time_scaled = ifelse(is_pseudodiploid,
                             (left_time_boundary / pseudodip_mu) * gen,
                             time_scaled),
        ne_scaled   = ifelse(is_pseudodiploid,
                             (1 / lambda_00) / (2 * pseudodip_mu),
                             ne_scaled)
      )
  }

  # Overlay autosomal curves
  if (overlay_autosomes && !is.null(autosomal_data)) {

    ### CHANGED: restored lambda_00 usage
    autos <- autosomal_data %>%
      mutate(
        time_scaled = (left_time_boundary / mu) * gen,
        ne_scaled   = (1 / lambda_00) / (2 * mu),
        is_pseudodiploid = FALSE,
        sample = paste0("autosome_", sample)
      )

    df <- bind_rows(df, autos)

    ### CHANGED: correct override logic retained
  ...
  ### REMOVED: highlight_flag creation
  ### REMOVED: highlight alpha logic
  ### REASON: we now simply subset before calling function
  ...
  p <- ggplot(df, aes(
    ...
    ### CHANGED: reinstated single-step geom
    geom_step(size = 1.1) +
    scale_color_manual(values = manual_colors)

  # Scales (with log ticks restored)
  if (!is.null(xlims)) {
    p <- p + scale_x_log10(limits = xlims)
  } else {
    p <- p + scale_x_log10()
  }
  ...
  # Line labels (unchanged except highlight logic removed)
  if (!is.null(line_label)) {

    ### CHANGED: simplified (no highlight condition)
    df <- df %>%
      mutate(
  ...
  # Theme — CHANGED: restored clean theme with dual-axis blanking
  p <- p +
