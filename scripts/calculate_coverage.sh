...
mkdir -p "$input_dir/validation/slurmout"
...
for bam_file in "$input_dir"/*.mdup.bam; do
	...
	base_name=$(basename "$bam_file" | sed 's/_mdup//; s/\.bam$//')
 	...
 	output_file="${output_dir}/WgsMetrics.${base_name}.txt"
 	...
 	sbatch --job-name="bamfile_validation_${base_name}" \
        	...
        	--output "$output_dir/slurmout/${base_name}.validate.o" \
         	...
         	--wrap="java -jar /gpfs/software/ada/picard/2.24.1/picard.jar CollectWgsMetrics I=$bam_file O=$output_file  R=$reference"
	#break 
done