...
plot_msmc_facet <- function(
  ...
  # --- Pseudodiploid + autosomal overlay ---
  if (!is.null(pseudodiploid_data) && !is.null(pseudodip_mu) && !is.null(autosomal_data)) {

    # Extract the two source individuals
    ind1 <- pseudodiploid_data$sample1[1]
    ind2 <- pseudodiploid_data$sample2[1]
    ...
    pseudo_line <- pseudodiploid_data %>%
      mutate(
        ...
        type        = "pseudodiploid",
        sample      = "pseudodiploid",  # hardcoded label
        continent   = "pseudodiploid",  # separate color category
    ...
    # Adjust grouping and color for overlay mode
    group_line_var <- "sample"    # one line per sample + pseudodiploid
    color_var      <- "continent" # autosomes use continent colors
    # *** KEY FIX: NO FACETING HERE ***
      # Force single-panel output for pseudodiploid comparison
      # -------------------------------
      panel_grouping_var <- NULL
      ncol_panels <- 1

      message("Pseudodiploid mode: forcing single-panel plot.")
  }
  ...
  if (is.null(manual_colors)) {
    ...
    if ("pseudodiploid" %in% df[[color_var]] && !"pseudodiploid" %in% names(manual_colors)) {
      manual_colors <- c(manual_colors, pseudodiploid = "black")
    }
  }
...
pseudodiploids_plots <- file.path(plots_dir, "msmc_pseudodiploids_individual_pairs")
if (!dir.exists(pseudodiploids_plots)) {
    dir.create(pseudodiploids_plots, recursive = TRUE)
}

pseudo_mu <- mu * 0.75  # Optional separate mu for pseudodiploid
...
for (pair_id in unique(msmc_pseudo$pair_id)) {
  ...
  # Extract source individuals for overlay
  ...
  plot_msmc_facet(
    msmc_data = autosomes_sub,         # autosomal lines
    autosomal_data = autosomes_sub,    # for overlay
    pseudodiploid_data = pseudo_pair,  # pseudodiploid overlay
    mu = mu,
    pseudodip_mu = pseudo_mu,
    gen = gen,
    panel_grouping_var = "pair_id",
    group_line_var = "sample",         # function will adjust for overlay
    color_var = "continent",           # autosomes get continent colors
    line_label = "sample + region",
    save_file = save_file,
    xlims = c(1e4, 1e7),
    ylims = c(1e3, 1e8)
